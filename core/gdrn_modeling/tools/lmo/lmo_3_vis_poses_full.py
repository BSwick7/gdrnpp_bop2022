...
model_paths = [
    osp.join(model_dir, f"obj_{obj_id:06d}.ply") for obj_id in id2obj]

ren = EGLRenderer(model_paths, vertex_scale=0.001,
                  use_cache=True, width=width, height=height)
...
vis_dir = (
    "output/gdrn/lmo_pbr/convnext_a6_AugCosyAAEGray_BG05_mlL1_DMask_amodalClipBox_classAware_lmo/inference_model_final_wo_optim/lmo_test/lmo_vis_gt_pred_full"
)
...
for d in tqdm(dset_dicts):
    ...
    save_path_0 = osp.join(
        vis_dir, "{}_{:06d}_vis0.png".format(scene_id, im_id))
