...
import numpy as np
import torch
from core.gdrn_modeling.engine.engine_utils import (batch_data_inference_roi,
                                                    get_out_coor, get_out_mask)
...
from lib.vis_utils.image import (vis_image_bboxes_cv2, vis_image_mask_bbox_cv2,
                                 vis_image_mask_cv2)
...
from core.gdrn_modeling.models import (GDRN, GDRN_cls, GDRN_cls2reg,  # noqa
                                       GDRN_double_mask,
...
class GdrnPredictor():
    def __init__(self,
                 config_file_path=osp.join(
                     PROJ_ROOT, "configs/gdrn/lmo_pbr/convnext_a6_AugCosyAAEGray_BG05_mlL1_DMask_amodalClipBox_classAware_lmo.py"),
                 ckpt_file_path=osp.join(
                     PROJ_ROOT, "output/gdrn/lmo_pbr/convnext_a6_AugCosyAAEGray_BG05_mlL1_DMask_amodalClipBox_classAware_lmo/model_final.pth"),
                 camera_json_path=osp.join(
                     PROJ_ROOT, "datasets/BOP_DATASETS/lmo/camera.json"),
                 path_to_obj_models=osp.join(
                     PROJ_ROOT, "datasets/BOP_DATASETS/lmo/models")
                 ):
        ...
        if self.cfg.TEST.USE_DEPTH_REFINE:
            ...
            self.ren_models = load_models(
                model_paths=[os.path.join(
                    self.objs_dir, f"obj_{i:06d}.ply") for i in self.objs.keys()],
    ...
    def postprocessing(self, data_dict, out_dict):
        ...
        for i_inst in range(len(data_dict["roi_img"])):
            ...
            cur_res = {
                ...
                # xyxy
                "bbox_est": data_dict["bbox_est"][i_inst].detach().cpu().numpy(),
            }
            if self.cfg.TEST.USE_PNP:
                pose_est_pnp = get_pnp_ransac_pose(
                    self.cfg, data_dict, out_dict, i_inst, i_out)
    ...
    def process_depth_refine(self, inputs, out_dict):
        ...
        out_transes = out_dict["trans"].detach().to(
        ...
        for i, _input in enumerate([inputs]):
            ...
            for inst_i in range(len(_input["roi_img"])):
                ...
                depth_sensor_crop = cv2.resize(
                    _input['roi_depth'][inst_i].cpu().numpy().copy().squeeze(), (64, 64))
                ...
                for _ in range(cfg.TEST.DEPTH_REFINE_ITER):
                    ...
                    self.ren.draw_model(
                        self.ren_models[self.cls_names.index(cls_name)], pose_est)
                    ...
                    yx_coords = np.meshgrid(
                        np.arange(crop_res), np.arange(crop_res))
                    # (crop_res, crop_res, 2yx)
                    yx_coords = np.stack(yx_coords[::-1], axis=-1)
                    yx_ray_2d = (
                        yx_coords * query_img_norm[..., None]).sum(axis=(0, 1))  # y, x
    ...
    def normalize_image(self, cfg, image):
        ...
        image: CHW format
        """
        pixel_mean = np.array(try_get_key(
            cfg, "MODEL.PIXEL_MEAN", "pixel_mean")).reshape(-1, 1, 1)
        pixel_std = np.array(try_get_key(
            cfg, "MODEL.PIXEL_STD", "pixel_std")).reshape(-1, 1, 1)
    ...
    def preprocessing(self, outputs, image, depth_img=None):
        ...
        for key in roi_keys:
            ...
            if key in dataset_dict:
                batch[key] = torch.cat([dataset_dict[key]], dim=0).to(
                    device='cuda', dtype=dtype, non_blocking=True)

        batch["roi_cam"] = torch.cat([dataset_dict["cam"]], dim=0).to(
            'cuda', non_blocking=True)
        batch["roi_center"] = torch.cat(
            [dataset_dict["bbox_center"]], dim=0).to('cuda', non_blocking=True)
        batch["bbox_center"] = torch.cat([dataset_dict["bbox_center"]], dim=0).to(
            'cuda', non_blocking=True)  # roi_infos["bbox_center"]
    ...
    def _get_extents(self):
        ...
        for i, obj_name in self.objs.items():
            ...
            model = inout.load_ply(
                model_path, vertex_scale=self.args.vertex_scale)
    ...
    def setup(self, args):
        ...
        if cfg.get("EXP_NAME", "") == "":
            setproctitle("{}.{}".format(os.path.splitext(
                os.path.basename(args.config_file))[0], get_time_str()))
        else:
        ...
        bs_ref = cfg.SOLVER.get(
            "REFERENCE_BS", cfg.SOLVER.IMS_PER_BATCH)  # nominal batch size
        ...
        exp_id = "{}".format(os.path.splitext(
